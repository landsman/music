...
1. This project uses Deno as the runtime environment and Supabase for backend
   services.
2. The project is structured with a web frontend (React + Vite) and Supabase
   functions.
...
2. Install Supabase CLI: Follow the instructions at
...
Tests are located in the `supabase/functions/tests` directory. Each test file
should follow the naming convention `*-test.ts`.
...
Deno.test("delay function test", async () => {
  ...
  assertEquals(
    elapsed >= 95,
    true,
    `Expected delay of at least 95ms, but got ${elapsed}ms`,
  );
});
```
...
The project uses Deno's standard assertion library `@std/assert`. Common
assertions include:

- `assertEquals(actual, expected, message?)`: Asserts that `actual` and
  `expected` are equal
- `assertNotEquals(actual, expected, message?)`: Asserts that `actual` and
  `expected` are not equal
- `assertStrictEquals(actual, expected, message?)`: Asserts that `actual` and
  `expected` are strictly equal
...
formatting and linting before each commit.
